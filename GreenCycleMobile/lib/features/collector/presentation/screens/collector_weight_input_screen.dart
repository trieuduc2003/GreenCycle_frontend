...
import '../../../yard/presentation/screens/transaction_complete_screen.dart';


class CollectorWeightInputScreen extends StatefulWidget {
  final String token;
  final int orderId;

  const CollectorWeightInputScreen({
    super.key,
    required this.token,
  ...
  @override
  State<CollectorWeightInputScreen> createState() => _CollectorWeightInputScreenState();
}

class _CollectorWeightInputScreenState extends State<CollectorWeightInputScreen> {
  ...
  Future<void> _loadOrderDetails() async {
    try {
      final details = await _repo.getOrderDetails(
        token: widget.token,
  ...
  Future<void> _sendConfirmation() async {
    ...
    try {
      await _txRepo.initiatePickupTransaction(
        token: widget.token,
          ...
          builder: (_) => AlertDialog(
            ...
            title: const Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Khấu trừ phí tiện lợi (Pick-up 20%) & Khấu hao do ẩm/tạp chất (Tối đa 15%).',
